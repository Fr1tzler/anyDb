...
import { EntityFieldValueType, FieldValueType, RecordType, fieldIsBoolean, fieldIsNumber, fieldIsObject, fieldIsString } from '../types'
...
export class EntityRepository implements IEntityRepository {
  ...
  // todo refactor
  public async createOne(partialEntity: Partial<Entity>): Promise<Entity | null> {
    ...
    const [entity] = await this.dbQuery<EntityType>('INSERT INTO "Entities"("entitySchemaId") VALUES ($1) RETURNING *', [partialEntity.schemaId])
  ...
  public async listAll(offset: number = 0, limit: number = 20): Promise<IListAllResponse<Entity>> {
    const totalResult = await this.dbQuery<{ count: string }>('SELECT COUNT(*) AS "count" FROM "Entity"')
    const total = Number(totalResult[0].count)
    const rawResult = await this.dbQuery<EntityType>('SELECT * FROM "Entity" OFFSET $1 LIMIT $1', [offset, limit])
  ...
  public async getOne(entityId: string): Promise<Entity | null> {    
    const [entityBase] = await this.dbQuery<EntityType>('SELECT * FROM "Entity" WHERE "id" = $1', [entityId])
    if (!entityBase) {
      return null
    }
  ...
  public async updateOne(entityId: string, partialEntity: Partial<Entity>): Promise<Entity | null> {
    const [entityToUpdate] = await this.dbQuery<EntityType>('SELECT * FROM "Entity" WHERE "id" = $1', [entityId])
    if (!entityToUpdate) {
      return null
    }
    ...
    await this.dbQuery('UPDATE "Entity" SET "updatedAt" = current_timestamp(0) WHERE "id" = $1', [entityId])
    await this.dbQuery('DELETE FROM "FieldValue" WHERE "entityId" = $1 AND "schemaFieldId" = ANY($2)', [entityId, fieldValueFieldIdsToDelete])
  ...
  // todo refactor
  private async createEntityFields(fieldValues: Partial<FieldValueType>[]): Promise<void> {
    const parametersInsertionString = fieldValues.map((_, index) => `(
      $${8 * index + 1}, 
      $${8 * index + 2}, 
      $${8 * index + 3}, 
      $${8 * index + 4}, 
    ...
    `, fieldValues.map(el => el as FieldValueType & FieldValueValues).flatMap((fieldValue) => [
  ...
  private async getFieldsBySchemaIds(schemaIds: string[]): Promise<SchemaFieldType[]> {
    return await this.dbQuery<SchemaFieldType>('SELECT * FROM "SchemaField" WHERE "schemaFieldId" = ANY($1)', [schemaIds])
  }

  private async getFieldValuesByEntityIds(entityIds: string[]) {
    return await this.dbQuery<FieldValueType>('SELECT * FROM "FieldValue" where "entityId" = ANY($1)', [entityIds])
  }

  private getFieldValueFromObjectField(field: SchemaFieldType, value: EntityFieldValueType, otherFields: IOtherFields): Partial<FieldValueType> | null {
